...
@njit(fastmath=True)
def accelerate_conversion(image, width, height, color_coef, step):
    array_of_values = []
    for x in range(0, width, step):
        for y in range(0, height, step):
            r, g, b = image[x, y] // color_coef
            if r + g + b:
                array_of_values.append(((r, g, b), (x, y)))
    return array_of_values

class ArtConverter:
    def __init__(self, path=PATH, pixel_size=7, color_lvl=8) -> None:
        pg.init()
        self.path = path
        ...
        self.PALETTE, self.COLOR_COEF = self.create_palette()
    ...
    def draw_converted_image(self):
        self.image = self.get_image()
        array_of_values = accelerate_conversion(self.image, self.WIDTH, self.HEIGHT, self.COLOR_COEF, self.PIXEL_SIZE)
